...
abstract class ProductImporterOne
{
    ...
    public function saveQuantity($productId,$qta){
        ...
        StockAvailable::setQuantity($this->id_product, null,$qta, null);
        $this->object->update();
        return 2;
    }
...
//        $this->object->upc = $this->getUpc();
        ...
        if ($this->object->id) {
            ...
            if ($sync_category) {
                ...
                //print_r($this->getCategoryDefault());
...
//                else
                    ...
                    //print_r($this->product->id_category_default);
            ...
            if (!empty($url)) {
                ...
                if (($fieldError = $image->validateFields($this->unfriendly_error, true)) === true and ($langFieldError = $image->validateFieldsLang($this->unfriendly_error, true)) === true and $image->add()) {
                    if (!self::copyImg($this->object->id, $url, $image->id)) {
                        $_warnings[] = Tools::displayError('Error copying image: ').print_r($url);
                    }
                } else {
    ...
    private function getLocalCategoryId($localcategoryname){
        global $ps_category_lang;

        $local_ids = [];

        foreach($localcategoryname as $val){
            $found_key = array_search($val['name'], array_column($people, 'fav_color'));
            array_push($local_ids,$ps_category_lang[$found_key]['id_category']);
        }
...
//      return str_replace("\r\n", "<br>", strip_tags($this->object->short_description[$this->object->id_lang]));
    ...
    {
        $ean13 = $this->object->ean13;
        if(isset($this->object->ean13) && strlen($this->object->ean13) == 13){
            return $this->object->ean13;
        }else{
            echo "ean not valid:".$this->object->ean13;
            $ean13 = null;
        }
