...
class AliExpressAttributeRepository //extends Repository
{
    ...
    public function importSuperAttributes($superAttributes)
    {
        ...
        foreach ($superAttributes as $attributeData) {
            ...
            if (strtolower(substr($attributeData['title'], 0, -1)) != "colore") {
                $type = 'radio';
            }

            $title = filter_var(strtolower(substr($attributeData['title'], 0, -1)), FILTER_SANITIZE_STRING);




            foreach($attributeData['value'] as $val){
                $id_attr = $accessroyImport->insertAttributeValue('custom-'.$title, $val['name'],$type);
