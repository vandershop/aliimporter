...
class AccessoryImporterOne extends ProductImporterOne
{
    ...
    public function getAttributeIdByvalue($value){
		...
		JOIN `'._DB_PREFIX_.'attribute_group_lang` alg ON (a.`id_attribute_group` = alg.`id_attribute_group` AND al.`id_lang` = '.intval($id_lang).')
		WHERE al.`name` = "'.(string) $value.'" and alg.`name` LIKE "custom-%"
		LIMIT 1 ');
    }
